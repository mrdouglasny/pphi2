...
5. `cmmapApplyCmap`, a continuous linear map that evaluates a continuous multilinear map at a point
-/
...
theorem fstCmmap_norm [NormedRing A] [NormedAlgebra 𝕜 A] [NormOneClass A] [NormedRing B]
  ...
  · refine (fstCmmap 𝕜 A B).opNorm_le_bound (M := 1) (by norm_num) ?_
    ...
    rw [e]
    rw [fstCmmap_apply]
    simp only [Fin.isValue, Prod.mk.eta, ge_iff_le]
    exact norm_fst_le (z 0)
  · have lo := (fstCmmap 𝕜 A B).unit_le_opNorm (m := fun _ ↦ (1, 1)) ?_
    · rw [fstCmmap_apply, norm_one] at lo; assumption
    · rw [pi_norm_le_iff_of_nonneg]; · intro i; simp only [Prod.norm_def, norm_one]; norm_num
      · norm_num
...
/-- A term of the general `n`-linear map on `𝕜 × 𝕜`,
...
theorem termCmmap_apply [NormedAddCommGroup E] [NormedSpace 𝕜 E] [SMulCommClass 𝕜 𝕜 E]
  ...
  | succ n h =>
    ...
    by_cases nk : n < k
    · simp only [nk, ↓reduceIte]
      rw [fstCmmap_apply]
      have hle : n ≤ k := nk.le
      have hle' : n + 1 ≤ k := Nat.succ_le_iff.mpr nk
      rw [min_eq_right hle]
...
theorem termCmmap_norm (𝕜 : Type) [NontriviallyNormedField 𝕜] [NormedAddCommGroup E]
  ...
  | succ n nh =>
    ...
    by_cases nk : n < k
    · simp only [nk, ↓reduceIte, ge_iff_le] at tn ⊢
      rw [fstCmmap_norm] at tn; simp only [one_mul] at tn; exact _root_.trans tn nh
    · simp only [nk, ↓reduceIte, ge_iff_le] at tn ⊢
      rw [sndCmmap_norm] at tn; simp only [one_mul] at tn; exact _root_.trans tn nh
