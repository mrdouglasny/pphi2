...
#### 18. `configuration_borelSpace` (Convergence.lean:180)

**Statement**: Borel sigma-algebra on S'(R^d) = sigma-algebra from evaluation maps.

**Plan**:
1. Let B = Borel(S', weak-*), C = sigma-algebra from evaluation maps ev_f
2. **C subset B**: Each ev_f is continuous (by definition of weak-*), hence Borel-measurable. C is generated by these, so C subset B
3. **B subset C**: Open sets in weak-* topology are generated by {omega : |omega(f_i) - c_i| < eps_i, i=1..n}. These are manifestly in C. Since S' is Polish (axiom 4), topology has countable basis, so all open sets are in C

**Key Mathlib**: `MeasurableSpace.generateFrom`, `borel`, `Measurable.of_continuous`
**Depends on**: `configuration_polishSpace`
**Difficulty**: MEDIUM
...
#### 41. `configuration_polishSpace` (Convergence.lean:173)

**Statement**: S'(R^d) is a Polish space.

**Plan**:
1. Define Schwartz space S(R^d) with topology from seminorms p_{alpha,beta}
2. Prove S(R^d) is Frechet (complete metrizable locally convex TVS)
3. Prove S(R^d) is nuclear (inclusion S -> L^2 factors through Hilbert-Schmidt)
4. Prove S(R^d) is separable
5. Invoke theorem: weak-* dual of separable nuclear Frechet space is Polish

**Key Mathlib**: `SeparableSpace`, `PolishSpace`, TVS libraries
**Blocker**: Nuclear Frechet spaces — entirely missing from Mathlib
**Difficulty**: HARD
...
| 41 | `configuration_polishSpace` | Convergence | HARD | Nuclear Frechet spaces |
