...
| Axiom | File:Line | Rating | Vetting | Plan / Notes |
|---|---|---|---|---|
...
| `pphi2_nontriviality` | `Main.lean:128` | Phase 4 endpoint | — | Bridge to φ⁴; downstream of `same_continuum_measure`. |

### Continuum-limit machinery (3 axioms — abstract Mitoma/Prokhorov chassis)

| Axiom | File:Line | Rating | Vetting | Plan / Notes |
|---|---|---|---|---|
| `continuum_exponential_moment_bound` | `ContinuumLimit/AxiomInheritance.lean:123` | Design bridge | SA 2026-04-19 | Mixed `L¹`/Green `∫ exp(\|ωf\|) dμ ≤ exp(c₁∫\|f\| + c₂ G(f,f))`. |
| `canonical_continuumMeasure_cf_tendsto` | `ContinuumLimit/AxiomInheritance.lean:327` | Design bridge | SA 2026-04-19 | Coupled UV/IR bridge, CF convergence along `aₙ → 0`, `Nₙ aₙ → ∞`. |
| `continuum_exponential_clustering` | `ContinuumLimit/AxiomInheritance.lean:354` | Correct for P(Φ)₂ | Gemini 2026-03-07 | Spectral-gap input for continuum OS4. |
| `continuumLimit_nonGaussian` | `ContinuumLimit/Convergence.lean:256` | Phase 4 endpoint | — | Nontriviality of the limit. |

### Cluster A — Nelson exponential estimate (1 axiom — bridge)

| Axiom | File:Line | Rating | Vetting | Plan / Notes |
|---|---|---|---|---|
| `polynomial_chaos_exp_moment_bridge` | `NelsonEstimate/PolynomialChaosBridge.lean:116` | Standard (over-stated `∀ a > 0`) | DT 2026-05-08, DT 2026-05-10 (Step 1 plan rev 2) | **The T² interacting OS critical-path axiom.** Plans: [parent](polynomial-chaos-exp-moment-bridge-proof-plan.md), [Step 1](rough-error-variance-plan.md) (rev 2 incorporates Gemini DT 2026-05-10 critique: K-quantifier hygiene, m=1 L¹×L^∞ bound replacing C-S, m≥2 L^m sum bound replacing ‖C_R‖_∞, RHS = `K·T·(1+|log T|)^{P.n−1}`, three named upstream sorries for parallel-tracked Glimm-Jaffe Ch. 8 Fourier estimates). [Review record](rough-error-variance-deep-think-review.md). |
