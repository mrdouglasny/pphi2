...
| **5** rough error variance | `RoughErrorBound.lean:1640–3800` (`rough_error_variance`) | ✅ Done (`AsymRoughErrorVariance.lean` `e33cef7` + cross-term L² discharge via `AsymCrossTermL2Identity.lean`) | `asymRoughError_variance` and `asymCanonicalCrossTerm_l2_sq_le` both proved with `#axioms = [propext, Classical.choice, Quot.sound]`; the cross-term L² covSum identity ported in full via gamma defs + parallel smooth/rough site-variance translation-invariance chain + 6 two-site marginal Wick lemmas through the now-public `wickPower_two_site_pi_gaussianReal_*` index-polymorphic helpers |
