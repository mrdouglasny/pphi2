...
| **L4** | interacting → free pull-back | `⟨\|X\|⟩_t = ∫\|X\|e^{−tV}/Z_t ≤ ‖X‖_{L²(μ_GFF)}·√(∫e^{−2tV})/Z_t ≤ ‖X‖_{L²}·√(max(1,K))` (`X = φ(f)ⁿV^k`), via **Cauchy–Schwarz + Nelson exp-estimate** (`∫e^{−2tV} ≤ K^t`, Hölder) and `Z_t ≥ 1` — NOT a pointwise `V ≥ −B` bound (that is false, see correction above) | **NEW** — needs `Z_t ≥ 1` (Jensen, `⟨V⟩₀=0`) + `∫e^{−2tV} ≤ K^t` (Hölder from `nelson_exponential_estimate`) |
